...
then
   ...
   if [ "$9" == "false" ]; then
    ...
    PYTHONPATH="$PWD/src" srun --hint=memory_bound -p $7 --gres=gpu:rtx2080ti:1 --reservation=triesch-shared --gpus-per-task=1 --mem $5 --time 29-00 python -m net.xstrct_run -c $3 $TESTFLAG
   fi    
fi
